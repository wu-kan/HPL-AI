...
#else
static void HPL_AI_agemv0( TRANS, M, N, ALPHA, A, LDA, X, INCX, BETA, Y, INCY )
   const enum HPL_TRANS       TRANS;
   const int                  INCX, INCY, LDA, M, N;
   const HPL_AI_T_afloat               ALPHA, BETA;
   const HPL_AI_T_afloat               * A, * X;
   HPL_AI_T_afloat                     * Y;
#endif
{
/*
 ...
 */
   ...
   if( ALPHA == HPL_rzero ) { HPL_AI_ascal( M, BETA, Y, INCY ); return; }
 
   if( TRANS == HplNoTrans )
   {
      HPL_AI_ascal( M, BETA, Y, INCY );
 ...
 * =======
 *
 * HPL_AI_agemv performs one of the matrix-vector operations
 *  
 ...
 *         least MAX(1,m).
 *
 * X       (local input)                 const HPL_AI_T_afloat *
 ...
 *
 * Y       (local input/output)          HPL_AI_T_afloat *
 ...
 */ 
   ...
   {
      HPL_AI_agemv0( TRANS, M, N, ALPHA, A, LDA, X, INCX, BETA, Y, INCY );
   }
   else
   {
      HPL_AI_agemv0( ( TRANS == HplNoTrans ? HplTrans : HplNoTrans ),
