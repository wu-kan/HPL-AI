...
#ifdef STDC_HEADERS
   int HPLAI_binit_blonM(
       HPLAI_T_panel *PANEL)
#else
int HPLAI_binit_blonM(PANEL)
    HPLAI_T_panel *PANEL;
#endif
   {
      /* ..
...
#define _M_COUNT_S1 (1LL * sizeof(HPLAI_T_AFLOAT) * PANEL->len)
#define _M_TYPE_S1 MPI_BYTE

#define _M_BUFF_S2 (void *)(PANEL->L2 + ibuf)
#define _M_COUNT_S2 (1LL * sizeof(HPLAI_T_AFLOAT) * lbuf)
#define _M_TYPE_S2 MPI_BYTE

#define _M_BUFF_R1 (void *)(PANEL->L2)
#define _M_COUNT_R1 (1LL * sizeof(HPLAI_T_AFLOAT) * PANEL->len)
#define _M_TYPE_R1 MPI_BYTE

#define _M_BUFF_R2 (void *)(PANEL->L2 + ibuf)
#define _M_COUNT_R2 (1LL * sizeof(HPLAI_T_AFLOAT) * lbuf)
#define _M_TYPE_R2 MPI_BYTE

#define _M_ROLL_BUFF_S (void *)(PANEL->L2 + ibufS)
#define _M_ROLL_COUNT_S (1LL * sizeof(HPLAI_T_AFLOAT) * lbufS)
#define _M_ROLL_TYPE_S MPI_BYTE
#define _M_ROLL_BUFF_R (void *)(PANEL->L2 + ibufR)
#define _M_ROLL_COUNT_R (1LL * sizeof(HPLAI_T_AFLOAT) * lbufR)
#define _M_ROLL_TYPE_R MPI_BYTE

#endif

#ifdef STDC_HEADERS
   int HPLAI_bcast_blonM(
       HPLAI_T_panel *PANEL,
       int *IFLAG)
#else
int HPLAI_bcast_blonM(PANEL, IFLAG)
    HPLAI_T_panel *PANEL;
int *IFLAG;
#endif
   {
...
#ifdef HPL_USE_MPI_DATATYPE
         if (ierr == MPI_SUCCESS)
            ierr = HPLAI_packL(PANEL, 0, PANEL->len, I_SEND);
#endif
         if (ierr == MPI_SUCCESS)
 ...
 */
         ...
         { /* if panel is here, proceed */
            ...
            {
#ifdef HPL_USE_MPI_DATATYPE
               ierr = HPLAI_packL(PANEL, 0, PANEL->len, I_RECV);
#endif
               if (ierr == MPI_SUCCESS)
 ...
 */
      prev = MModSub1(rank, size);
      if (MModSub1(prev, size) == root)
         prev = root;
      next = MModAdd1(rank, size);
      if (rank == root)
         next = MModAdd1(next, size);
...
#ifdef STDC_HEADERS
   int HPLAI_bwait_blonM(
       HPLAI_T_panel *PANEL)
#else
int HPLAI_bwait_blonM(PANEL)
    HPLAI_T_panel *PANEL;
#endif
   {
      /* ..
