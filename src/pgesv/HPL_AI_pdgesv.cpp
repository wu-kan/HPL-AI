    ...
    const int MAXIT)
{
   ...
   *         array information. 
   * FA      (local input/output)          HPL_AI_T_pmat *
   *         On entry, A points to the data structure containing the local
...
void HPL_AI_pdgesv(
    ...
    HPL_T_pmat *A)
#else
void HPL_AI_pdgesv(GRID, ALGO, A)
    HPL_T_grid *GRID;
HPL_T_palg *ALGO;
HPL_AI_T_pmat *A;
#endif
{
    ...
    FA.A = (HPL_AI_T_afloat *)HPL_PTR(vptr_FA, ((size_t)(ALGO->align) * sizeof(HPL_AI_T_afloat)));

    FA.X = Mptr(FA.A, 0, A->nq, A->ld);

    HPL_T_pmat_cpy(&FA, A);

//    HPL_AI_pagesv(GRID, ALGO, &FA);
    {
        HPL_T_pmat TA;
