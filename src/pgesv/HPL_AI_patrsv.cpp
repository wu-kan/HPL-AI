 ...
 *
 * AMAT    (local input/output)          HPL_AI_T_pmat *
 ...
 */
   ...
   HPL_AI_T_afloat                     * A=NULL, * Aprev=NULL, * Aptr, * XC=NULL,
 ...
 */
   ...
   Aptr = (HPL_AI_T_afloat *)(A); XC = Mptr( Aptr, 0, Anq, lda );
   Mindxg2p( n, nb, nb, Bcol, 0, npcol );

   if( ( Anp > 0 ) && ( Alcol != Bcol ) )
   {
      if( mycol == Bcol  )
      { (void) HPL_AI_send( XC, Anp, Alcol, Rmsgid, Rcomm ); }
      else if( mycol == Alcol )
      { (void) HPL_AI_recv( XC, Anp, Bcol,  Rmsgid, Rcomm ); }
   }
   Rmsgid = ( Rmsgid + 2 >
 ...
 */
   ...
   if( Anp > 0 )
   {
      W = (HPL_AI_T_afloat*)malloc( (size_t)(Mmin( n1, Anp )) * sizeof( HPL_AI_T_afloat ) );
      if( W == NULL )
      { HPL_pabort( __LINE__, "HPL_AI_patrsv", "Memory allocation failed" ); }
      Wfr = 1;
   }
   ...
   if( mycol == Alcol )
   {
      ...
      if( myrow == Alrow )
      {
         HPL_AI_atrsv( HplColumnMajor, HplUpper, HplNoTrans, HplNonUnit,
                    kb, Aptr+Anp, lda, XC+Anp, 1 );
         HPL_AI_acopy( kb, XC+Anp, 1, Xd, 1 );
      }
   }
 ...
 */
         ...
         {
            if( GridIsNot1xQ )
               (void) HPL_AI_send( Xdprev, kbprev, MModSub1( myrow, nprow ),
                                Cmsgid, Ccomm );
         }
         else
         {
            (void) HPL_AI_recv( Xdprev, kbprev, MModAdd1( myrow, nprow ),
                             Cmsgid, Ccomm );
         } 
/*
 ...
 */
         if( n1pprev > 0 )
         {
            ...
            HPL_AI_agemv( HplColumnMajor, HplNoTrans, n1pprev, kbprev,
                       -HPL_rone, Aprev+tmp1, lda, Xdprev, 1, HPL_rone,
                       XC+tmp1, 1 );
            if( GridIsNotPx1 )
               (void) HPL_AI_send( XC+tmp1, n1pprev, Alcol, Rmsgid, Rcomm );
         }
/*
 ...
 */
         if( n1pprev > 0 )
         {
            (void) HPL_AI_recv( W, n1pprev, colprev, Rmsgid, Rcomm );
            HPL_AI_aaxpy( n1pprev, HPL_rone, W, 1, XC+Anpprev-n1pprev, 1 );
         }
      }
/*
 ...
 */
      if( ( mycol == Alcol ) && ( myrow == Alrow ) )
      {
         HPL_AI_atrsv( HplColumnMajor, HplUpper, HplNoTrans, HplNonUnit,
                    kb, Aptr+Anp, lda, XC+Anp, 1 );
         HPL_AI_acopy( kb, XC+Anp, 1, XR+Anq, 1 );
      }
/*
*  Finish previous update
*/
      if( ( mycol == colprev ) && ( ( tmp1 = Anpprev - n1pprev ) > 0 ) )
         HPL_AI_agemv( HplColumnMajor, HplNoTrans, tmp1, kbprev, -HPL_rone,
 ...
 */
   if( mycol == colprev )
      (void) HPL_AI_broadcast_afloat( (void *)(XR), kbprev, rowprev,
                            Ccomm );
