 ...
 */
   ...
   {
      if( ( myrow == 0 ) && ( mycol == 0 ) )
         HPL_pwarn( TEST->outfp, __LINE__, "HPL_AI_pdtest",
                    "[%d,%d] %s", info[1], info[2],
 ...
 */
   ...
   HPL_AI_pdmatgen( GRID, N, N+1, NB, mat.A, mat.ld, HPL_ISEED );
 ...
 */
      ...
      if( wtime[0] > HPL_rzero ) {
         ...
         HPL_fprintf( TEST->outfp,
             "HPL_AI_pdgesv() start time %s\n", ctime( &current_time_start ) );
         HPL_fprintf( TEST->outfp,
             "HPL_AI_pdgesv() end time   %s\n", ctime( &current_time_end ) );
      }
   }
#ifdef HPL_DETAILED_TIMING
   HPL_ptimer_combine( GRID->all_comm, HPL_AMAX_PTIME, HPL_WALL_PTIME,
 ...
 */
   ...
   {
      if( ( myrow == 0 ) && ( mycol == 0 ) )
         HPL_pwarn( TEST->outfp, __LINE__, "HPL_AI_pdtest", "%s %d, %s", 
