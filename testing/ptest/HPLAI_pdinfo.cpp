 ...
 */
      ...
      (void) fgets( line, HPLAI_LINE_MAX - 2, infp ); lineptr = line;
      for( i = 0; i < *NS; i++ )
      {
 ...
 */
      ...
      for( i = 0; i < *NPFS; i++ )
      {
         (void) sscanf( lineptr, "%s", num ); lineptr += strlen( num ) + 1;
         j = atoi( num );
         if(      j == 0 ) PF[ i ] = HPLAI_LEFT_LOOKING;
         else if( j == 1 ) PF[ i ] = HPLAI_CROUT;
         else if( j == 2 ) PF[ i ] = HPLAI_RIGHT_LOOKING;
         else              PF[ i ] = HPLAI_RIGHT_LOOKING;
      }
/*
 ...
 */
      ...
      for( i = 0; i < *NRFS; i++ )
      {
         (void) sscanf( lineptr, "%s", num ); lineptr += strlen( num ) + 1;
         j = atoi( num );
         if(      j == 0 ) RF[ i ] = HPLAI_LEFT_LOOKING;
         else if( j == 1 ) RF[ i ] = HPLAI_CROUT;
         else if( j == 2 ) RF[ i ] = HPLAI_RIGHT_LOOKING;
         else              RF[ i ] = HPLAI_RIGHT_LOOKING;
      }
/*
 ...
 */
      ...
      for( i = 0; i < *NTPS; i++ )
      {
         (void) sscanf( lineptr, "%s", num ); lineptr += strlen( num ) + 1;
         j = atoi( num );
         if(      j == 0 ) TP[ i ] = HPLAI_1RING;
         else if( j == 1 ) TP[ i ] = HPLAI_1RING_M;
         else if( j == 2 ) TP[ i ] = HPLAI_2RING;
         else if( j == 3 ) TP[ i ] = HPLAI_2RING_M;
         else if( j == 4 ) TP[ i ] = HPLAI_BLONG;
         else if( j == 5 ) TP[ i ] = HPLAI_BLONG_M;
         else              TP[ i ] = HPLAI_1RING_M;
      }
/*
 ...
 */
      ...
      if(      j == 0 ) *FSWAP = HPLAI_SWAP00;
      else if( j == 1 ) *FSWAP = HPLAI_SWAP01;
      else if( j == 2 ) *FSWAP = HPLAI_SW_MIX;
      else              *FSWAP = HPLAI_SWAP01;
/*
 ...
 */
   ...
   if( rank == 0 )
   {
      ...
      for( i = 0; i < *NPFS; i++ )
      {
         if(      PF[i] == HPLAI_LEFT_LOOKING  ) iwork[j] = 0;
         else if( PF[i] == HPLAI_CROUT         ) iwork[j] = 1;
         else if( PF[i] == HPLAI_RIGHT_LOOKING ) iwork[j] = 2;
         j++;
      }
      ...
      for( i = 0; i < *NRFS; i++ )
      {
         if(      RF[i] == HPLAI_LEFT_LOOKING  ) iwork[j] = 0;
         else if( RF[i] == HPLAI_CROUT         ) iwork[j] = 1;
         else if( RF[i] == HPLAI_RIGHT_LOOKING ) iwork[j] = 2;
         j++;
      }
      for( i = 0; i < *NTPS; i++ )
      {
         if(      TP[i] == HPLAI_1RING   ) iwork[j] = 0;
         else if( TP[i] == HPLAI_1RING_M ) iwork[j] = 1;
         else if( TP[i] == HPLAI_2RING   ) iwork[j] = 2;
         else if( TP[i] == HPLAI_2RING_M ) iwork[j] = 3;
         else if( TP[i] == HPLAI_BLONG   ) iwork[j] = 4;
         else if( TP[i] == HPLAI_BLONG_M ) iwork[j] = 5;
         j++;
      }
      for( i = 0; i < *NDHS; i++ ) { iwork[j] = DH[i]; j++; }

      if(      *FSWAP == HPLAI_SWAP00 ) iwork[j] = 0;
      else if( *FSWAP == HPLAI_SWAP01 ) iwork[j] = 1;
      else if( *FSWAP == HPLAI_SW_MIX ) iwork[j] = 2;
      j++;
   }
   ...
   if( rank != 0 )
   {
      ...
      for( i = 0; i < *NPFS; i++ )
      {
         if(      iwork[j] == 0 ) PF[i] = HPLAI_LEFT_LOOKING;
         else if( iwork[j] == 1 ) PF[i] = HPLAI_CROUT;
         else if( iwork[j] == 2 ) PF[i] = HPLAI_RIGHT_LOOKING;
         j++;
      }
      ...
      for( i = 0; i < *NRFS; i++ )
      {
         if(      iwork[j] == 0 ) RF[i] = HPLAI_LEFT_LOOKING;
         else if( iwork[j] == 1 ) RF[i] = HPLAI_CROUT;
         else if( iwork[j] == 2 ) RF[i] = HPLAI_RIGHT_LOOKING;
         j++;
      }
      for( i = 0; i < *NTPS; i++ )
      {
         if(      iwork[j] == 0 ) TP[i] = HPLAI_1RING;
         else if( iwork[j] == 1 ) TP[i] = HPLAI_1RING_M;
         else if( iwork[j] == 2 ) TP[i] = HPLAI_2RING;
         else if( iwork[j] == 3 ) TP[i] = HPLAI_2RING_M;
         else if( iwork[j] == 4 ) TP[i] = HPLAI_BLONG;
         else if( iwork[j] == 5 ) TP[i] = HPLAI_BLONG_M;
         j++;
      }
      for( i = 0; i < *NDHS; i++ ) { DH[i] = iwork[j]; j++; }

      if(      iwork[j] == 0 ) *FSWAP = HPLAI_SWAP00;
      else if( iwork[j] == 1 ) *FSWAP = HPLAI_SWAP01;
      else if( iwork[j] == 2 ) *FSWAP = HPLAI_SW_MIX;
      j++;
   }
   if( iwork ) free( iwork );
/*
