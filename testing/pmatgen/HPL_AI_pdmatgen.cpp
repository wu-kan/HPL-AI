...
void HPL_AI_pdmatgen(
    ...
    const int ISEED)
#else
void HPL_AI_pdmatgen(GRID, M, N, NB, A, LDA, ISEED)
    const HPL_T_grid *GRID;
const int M;
const int N;
const int NB;
 ...
 */
   ...
   if (N == M + 1)
   {
      HPL_AI_generateA(GRID, M, NB, A, LDA);
      int nq = HPL_numroc(M, NB, NB, mycol, 0, npcol);
      HPL_AI_generateB(GRID, M, NB, Mptr(A, 0, nq, LDA), ISEED);
      return;
   }
